...
export default function Preview() {
  ...
  const imageComponents = [Img1, Img2, Img3, Img4];
  const textComponents  = [Text1, Text2, Text3, Text4];
  const totalSlides = 4;
  ...
  const goToLogin = () => router.replace('/auth/sign-in');

  const nextSlide = () => {
    if (currentIndex < totalSlides - 1) {
      animateSlide('left');
    } else {
      goToLogin(); // último slide -> login
    }
  };
      ...
      <View className="flex-1">
        {currentIndex >= 1 ? (
          <View className="bg-[#F7FFED] w-full h-[8%] flex flex-row justify-between items-end px-4">
            <Pressable onPress={() => animateSlide('right')} hitSlop={12}>
          ...
          <View className="bg-[#F7FFED] w-full h-[8%]" />
        )}
        ...
        {/* Barra inferior com indicadores + botão next */}
