  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await getUserFriends(String(id));
        if (mounted) setFriends(list);
      } catch (e: any) {
        // amigos são “opcional”; não precisa travar a tela
        console.warn("Erro ao buscar amigos:", e?.message);
      } finally {
        ...
        return (
          <Friends
            friends={(friends ?? []).map((f) => ({
              ...
              name: f.name ?? "—",
              city: f.city ?? "—",
              // seu componente FriendCard usa imagens locais;
              // se quiser usar foto URL, adapte o componente.
              avatar: "user1", // placeholder (ou ajuste FriendCard para aceitar URL)
              mutualAvatars: ["user2", "user3"],
              mutualCount: f.mutualCount ?? 0,
            }))}
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
          <ProfileHeader
            ...
            stats={{ friends: (friends ?? []).length, activities: participated?.length ?? 0, createdActivities: 0 }}
            rating="4.80"
          />
