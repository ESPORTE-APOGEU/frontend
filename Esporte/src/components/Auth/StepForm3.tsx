...
export default function StepForm3({ form, setForm }: StepsSignupProps) {
  ...
  const checkProfileExists = async (): Promise<
    "exists" | "missing" | "unauth" | "error"
  > => {
    try {
  ...
  const handleCreateAccount = async () => {
    try {
      ...
      if (!signUp || !setActive)
        throw new Error("Clerk SignUp não inicializado.");
  ...
  const handleConfirmCode = async () => {
    try {
      ...
      if (!code)
        throw new Error("Informe o código que enviamos ao seu e-mail.");
      ...
      if (state === "unauth") {
        throw new Error(
          "Sessão inválida após verificação. Tente entrar novamente."
        );
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Autocomplete
          label="Cidade"
          value={selectedCity} // <<--- passe o valor selecionado
        ...
        <View className="mt-8">
          <LargeButton
            ...
            title={loading ? "Enviando..." : "Criar Conta"}
          />
        </View>
      </ScrollView>

      {/* Modal (overlay mais robusto no iOS) */}
      <Modal
        ...
        <View className="flex-1 bg-black/40 items-center justify-center px-6">
          <View className="w-full bg-white rounded-2xl p-5">
            ...
            <Text className="text-gray-600 mb-4">
              ...
              <Text className="font-semibold">{form.email}</Text>. Digite-o
